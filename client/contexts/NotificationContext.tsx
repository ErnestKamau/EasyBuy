...
export function NotificationProvider({
  ...
  const updatePreference = useCallback(
    async (type: string, enabled?: boolean, pushEnabled?: boolean) => {
      try {
  ...
  useEffect(() => {
    ...
    websocketService.connect(user.id, user.role || "user").catch((error) => {
      console.error(
        "Failed to connect WebSocket, falling back to polling:",
        error
      );
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      refreshNotifications();
    }, 30000) as unknown as NodeJS.Timeout; // Poll every 30 seconds
        ...
        if (data?.order_id) {
          ...
          const orderId =
            typeof data.order_id === "number"
              ? data.order_id
              : Number.parseInt(String(data.order_id), 10);
