...
class AssignDriverAction
{
     ...
     *  4. Send Expo push notification to the driver's device.
    ...
    public function execute(Order $order, User $driver): Order
    {
        ...
        // 3. Push notification to driver's device (Expo via device_tokens)
        NotificationService::create(
            $driver->id,
            'delivery_assigned',
            'New Delivery Assigned!',
            "Order #{$order->order_number} is ready. Tap to accept.",
            [
                'type'         => 'new_delivery',
                'order_id'     => $order->id,
